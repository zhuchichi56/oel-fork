...
if [ "$USE_BSL" = "true" ]; then
    ...
    model_path="/mnt/zhuhe/saves/oel/${EXP_NAME}/global_step_${CKPT}/actor/huggingface"
    mkdir -p /mnt/zhuhe/saves/oel/${EXP_NAME}/global_step_${CKPT}/actor/huggingface/
    find /mnt/zhuhe/saves/oel/${EXP_NAME}/global_step_${CKPT}/actor/ -maxdepth 1 -type f ! -name "*.pt" -exec cp {} /mnt/zhuhe/saves/oel/${EXP_NAME}/global_step_${CKPT}/actor/huggingface/ \;
    python tools/merge_model2hf.py --local_dir /mnt/zhuhe/saves/oel/${EXP_NAME}/global_step_${CKPT}/actor
    final_model_path=$model_path
fi
...
if [ "${OMPI_COMM_WORLD_RANK:-0}" -eq 0 ]; then
    python3 -m verl.trainer.main_ppo \
        ...
        trainer.validation_data_dir=/mnt/zhuhe/saves/oel/${EXP_NAME}/global_step_${CKPT}/evaluation
else
    sleep infinity
fi